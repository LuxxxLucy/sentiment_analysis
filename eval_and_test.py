...
outputColumnIndex = 1
def eval(name,pos):
    gold = open(name+r"/dev.out", "r", encoding='UTF-8')
    prediction = open(name+r"/dev.p"+str(pos)+".out", "r", encoding='UTF-8')
...
def eval_v2(name):
    gold = open(name+r"/data.test.out", "r", encoding='UTF-8')
    prediction = open(name+r"/test.out", "r", encoding='UTF-8')
...
def test():
    for i in ["CN","EN","ES","SG"]:
        print("\n============\nlanguage "+i)
        for pos in [2,3,4,5,]:
            print("\n------------\nresult for part "+str(pos))
            eval(i,pos)
    for name in ["EN","ES"]:
        print("\n============\ntest result language "+i)
        eval_v2(name)
